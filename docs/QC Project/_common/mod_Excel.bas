...
On Error GoTo ERR_EXP
    ...
    pBar.Value = 0
    pBar.Visible = True
    sString = FormatNomeFile(Replace("QC XXX data base ", "XXX", DatabaseName) & FormatDateTime(Now, vbShortDate) & " rel" & dbCodeRelease)
...
Public Function dbChemicalFGToExcel(ByRef pBar As ProgressBar) As Boolean
Dim rc As Boolean
Dim sString As String
On Error GoTo ERR_EXP
    rc = True
...
Public Function dbChemicalQCToExcel(ByRef pBar As ProgressBar) As Boolean
Dim rc As Boolean
Dim sString As String
On Error GoTo ERR_EXP
    rc = True
    pBar.Value = 0
    pBar.Visible = True
    sString = FormatNomeFile("QC data base " & FormatDateTime(Now, vbShortDate) & " rev 2")
    If CreateExcel(False) Then
        NewExcelWorksheet (sString)
        If CopyHannaCodeData(pBar) Then
            Call SaveHannaCodeExcel(sString)
...
Public Function EsportaExcel(ByVal FileName As String, ByVal sString As String) As Boolean
    Dim rc As Boolean
    Dim ExcelPageName As String
   ...
   If SettingName = "" Then Exit Function
        ...
        If CreateExcel(False) Then
            ExcelPageName = Left(sString, 20)
          
            NewExcelWorksheet (ExcelPageName)
            If CopyChemicalQCData(SettingName) Then
...
ERR_IMP:
    ...
    Resume ERR_END
End Function
Private Function SaveExcel(ByVal sString As String)
Dim Mystring As String

    Mystring = USER_EXCEL_PATH & "\" & FormatNomeFile(sString) & "_QC" & ".xlsx"
...
End Function
Private Function CreateExcel(Optional ByVal bValue As Boolean = True) As Boolean
    Dim rc As Boolean
...
Private Sub NewExcelWorksheet(sString As String)
...
ERR_ADD:
    ...
    Resume Next
End Function
Public Function AddValue(Col As Long, Row As Integer, Text As String, Optional USER_COLOR As Boolean = False, Optional bTitle As Boolean = False, Optional Color As OLE_COLOR, Optional bNonSelezionati As Boolean, Optional DecimalFormat As Boolean) As Boolean
    Dim rc As Boolean
    Dim bValue As Boolean
    Dim Intervallo As Excel.Range
    Dim sRange As String
    ...
    COLOR_BACK = vbColorTextLightBlue
    COLOR_FORE = vbWhite
    ...
    If DecimalFormat Then
        Intervallo.NumberFormat = "Decimal"
    End If
    
    Intervallo.Value = CStr(Text)
...
End Sub
Private Function CloseExcel() As Boolean
    Dim rc As Boolean
