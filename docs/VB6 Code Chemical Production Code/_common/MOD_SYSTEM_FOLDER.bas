...
Public Const PROGRAM_NAME As String = "ChemicalProduction"
...
Public USER_DATA_PATH As String ' Recipe for production
Public USER_TEMP_PATH As String ' Recipe for production
Public USER_PREPARATION_PATH As String
Public USER_PRODUCTION_PATH As String
...
Public PathPrepRequisition As String
...
a = 2
   ...
   PathRequisition = "Material Requisition (" & Right$(year(Date), 2) & ")"
   PathPrepRequisition = "Material Requisition Preparation (" & Right$(year(Date), 2) & ")"
   
    If FSO.FolderExists(USER_DOCUMENTI & PathRequisition) = False Then
        FSO.CreateFolder USER_DOCUMENTI & PathRequisition
    End If
    
    If FSO.FolderExists(USER_DOCUMENTI & PathPrepRequisition) = False Then
        FSO.CreateFolder USER_DOCUMENTI & PathPrepRequisition
    End If
    
   PathRecipe = "Recipe"
   
    If FSO.FolderExists(USER_DOCUMENTI & PathRecipe) = False Then
            FSO.CreateFolder USER_DOCUMENTI & PathRecipe
    End If
        
    PathRecipe = USER_DOCUMENTI & PathRecipe & "\"
...
a = 3
    ...
    'If DirExists(USER_DOCUMENTI & "Preparation") = False Then
        MakePath (USER_DOCUMENTI & "Preparation")
        MakePath (USER_DOCUMENTI & "Production")
        
        MakePath (USER_DOCUMENTI & "Preparation\Data")
        MakePath (USER_DOCUMENTI & "Production\Data")
    'End If
    
    

 USER_PREPARATION_PATH = USER_DOCUMENTI & "Preparation\"
 USER_PRODUCTION_PATH = USER_DOCUMENTI & "Production\"
...
ERR_END:
    On Error GoTo 0
    Load_Folder = rc
    SaveSetting App.Title, "PATH", "Folder ChemicalProduction", rc
    Exit Function
ERR_FOLDER:
    rc = False
