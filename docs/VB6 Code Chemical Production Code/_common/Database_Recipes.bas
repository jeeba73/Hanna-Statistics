...
Public Sub CopyRecipeGrid2(ByVal Grd2 As Grid, ByVal lId As Long)
    ...
    With Grd2
      ...
      ' Next
        ...
        .Cell(1, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Code)), "", Trim(dbTabRecipe!Code))
        .Cell(2, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Description)), "", Trim(dbTabRecipe!Description))
        .Cell(3, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Line)), "", Trim(dbTabRecipe!Line))
        .Cell(4, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Rev)), "", Trim(dbTabRecipe!Rev)) '"  " & "Recipe Revision #"
        .Cell(5, 2).Text = IIf(IsNull(Trim(dbTabRecipe!RevDate)), "", Trim(dbTabRecipe!RevDate)) '"  " & "Rev Date"
        .Cell(6, 2).Text = IIf(IsNull(Trim(dbTabRecipe!NoteRev)), "", Trim(dbTabRecipe!NoteRev)) '"  " & "Note"
        .Cell(7, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Exp)), "", Trim(dbTabRecipe!Exp)) '"  " & "Exp ( Years )"
        .Cell(8, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Density)), "", Trim(dbTabRecipe!Density)) '"  " & "Density"
        .Cell(9, 2).Text = IIf(IsNull(Trim(dbTabRecipe!MaxQty)), "", Trim(dbTabRecipe!MaxQty)) '"  " & "Max Q.ty"
        .Cell(10, 2).Text = IIf(IsNull(Trim(dbTabRecipe!UmMax)), "", Trim(dbTabRecipe!UmMax)) ' "  " & "um Max Q.ty"
        .Cell(11, 2).Text = IIf(IsNull(Trim(dbTabRecipe!MinQty)), "", Trim(dbTabRecipe!MinQty)) '"  " & "Min Q.ty"
        .Cell(12, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Multiple)), "", Trim(dbTabRecipe!Multiple)) '"  " & "Multiple"
        .Cell(13, 2).Text = IIf(IsNull(Trim(dbTabRecipe!UmMultiple)), "", Trim(dbTabRecipe!UmMultiple)) '"  " & "um Multiple"
        .Cell(14, 2).Text = IIf(IsNull(Trim(dbTabRecipe!MinQty2)), "", Trim(dbTabRecipe!MinQty2)) '"  " & "Min Q.ty Multiple"
        .Cell(15, 2).Text = IIf(IsNull(Trim(dbTabRecipe!UmMinQty)), "", Trim(dbTabRecipe!UmMinQty)) '"  " & "um Min Q.ty"
        .Cell(16, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Mix)), "", Trim(dbTabRecipe!Mix)) '"  " & "Mix"
        .Cell(17, 2).Text = dbTabRecipe!bNoPreparation

        .Cell(18, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Classification)), "", Trim(dbTabRecipe!Classification)) ' "  " & "Classification"
        
        .Cell(19, 2).Text = IIf(IsNull(Trim(dbTabRecipe!Procedure)), "", Trim(dbTabRecipe!Procedure)) '"  " & "Procedure #"
        .Cell(20, 2).Text = IIf(IsNull(Trim(dbTabRecipe!ProcedureDate)), "", Trim(dbTabRecipe!ProcedureDate)) '"  " & "Procedure Date"
...
rc = True
    ...
    With dbTabRecipe
        ...
        !Description = Grd2.Cell(2, 2).Text
        !Line = Grd2.Cell(3, 2).Text
        !Rev = Grd2.Cell(4, 2).Text
        !RevDate = Grd2.Cell(5, 2).Text
        !NoteRev = Grd2.Cell(6, 2).Text
        !Exp = Grd2.Cell(7, 2).Text
        !Density = Grd2.Cell(8, 2).Text
        !MaxQty = Grd2.Cell(9, 2).Text
        !UmMax = Grd2.Cell(10, 2).Text
        !MinQty = Grd2.Cell(11, 2).Text
        !Multiple = Grd2.Cell(12, 2).Text
        !UmMultiple = Grd2.Cell(13, 2).Text
        !MinQty2 = Grd2.Cell(14, 2).Text
        !UmMinQty = Grd2.Cell(15, 2).Text
        !Mix = Grd2.Cell(16, 2).Text
        !bNoPreparation = Grd2.Cell(17, 2).Text
        !Classification = Grd2.Cell(18, 2).Text
        !Procedure = Grd2.Cell(19, 2).Text
        !ProcedureDate = Grd2.Cell(20, 2).Text
