Attribute VB_Name = "ChemicalProductionFunctions"
Option Explicit
...
Public Function SplitTextStringClassification(ByVal strObbligatoria As String, ByVal sString As String, ByRef DataType As Variant, ByRef Quanti As Integer) As Boolean
    Dim rc As Boolean
    Dim miaStringa As String
    Dim vettore As Variant
    Dim i As Integer, Somma As Long
...
ERR_END:
    On Error GoTo 0
    SplitTextStringClassification = rc
    Exit Function
ERR_CHECK:
    rc = False
    Resume Next
...
Public Function CheckRmxRecipeInRecipe(AR() As RmxRecipe, ByVal Val As String, ByVal RecipeCode As String, Optional ByVal haveMixes As Boolean, Optional ByVal MixCode As String) As Long
    Dim i As Long, ei As Long
    
    CheckRmxRecipeInRecipe = -1
    ...
    For i = 0 To ei
        Debug.Print AR(i).CHCode
        If AR(i).bMix Then
            If haveMixes Then
                If AR(i).CHCode = Val And RecipeCode <> AR(i).RecipeCode Then CheckRmxRecipeInRecipe = i: Exit For
            Else
                If AR(i).CHCode = Val And RecipeCode = AR(i).RecipeCode Then CheckRmxRecipeInRecipe = i: Exit For
            End If
        ...
        Else
            If haveMixes Then
                If AR(i).CHCode = Val And AR(i).RecipeCode = MixCode Then CheckRmxRecipeInRecipe = i: Exit For
            Else
                If AR(i).CHCode = Val Then CheckRmxRecipeInRecipe = i: Exit For
            End If
        End If
    Next
