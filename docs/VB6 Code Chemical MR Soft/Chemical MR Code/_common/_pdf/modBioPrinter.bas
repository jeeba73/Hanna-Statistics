...
Public Function PdfPrinterExist() As Boolean
    '------------------------------------------------
    ' BioPFD Printer
    '------------------------------------------------
...
SET_ERR:
    ...
    PopupMessage 3, "Attenzione stampante virtuale Bio-pdf non installata correttamente.", , True
    If InStr(UCase(MyErr), UCase("bioPDF")) Then
       If F_MsgBox.DoShow(err.Description & vbCrLf & "Installare il Driver della stampante virtuale Bio-PDF per generare Report e Certificati.", "Driver Stampante PDF") Then
        ...
        Else
            PopupMessage 3, "Sarà necessario Installare i driver (BioPDF) per poter creare i documenti PDF."
        End If
    Else
        PopupMessage 3, "Errore Stampante : " & MyErr, , True, "Driver Stampante PDF"
    End If
    Resume RESUME_ERR
End Function
...
If FileExists(App.PATH & "\PDFPrinter\Setup_bioPDFSetup_11_4_0_2674_PRO_EXP.exe") Then
    ...
    Else
        PopupMessage 3, "Attenzione impossibile trovare Setup_bioPDFSetup_11_4_0_2674_PRO_EXP.exe, Si consiglia di Reinstallare il programma.", , True
End If
