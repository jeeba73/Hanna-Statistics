    ...
    With Grid
        ...
        With dbTabPreparation
            ...
            For i = 1 To .RecordCount
                ...
                Grid.Cell(Grid.Rows - 1, 3).Text = IIf(IsNull(Trim(!Description)), "", Trim(!Description)) ' iPreparation(i).Recipe.Description
              ...
              '  If bHasMixes Then
                ...
                Grid.Cell(Grid.Rows - 1, 8).Alignment = cellRightCenter
                     ...
                     'grid.Cell(0, 1).Text = "Recipe"
                     'grid.Cell(0, 2).Text = "Description"
                     'grid.Cell(0, 3).Text = "Prep Week"
                     'grid.Cell(0, 4).Text = "Planned Prep."
                     'grid.Cell(0, 5).Text = "Data Recipe"
                     'grid.Cell(0, 6).Text = "Qty To Produce"
                    ' grid.Cell(0, 7).Text = "Qty Produced"
                    ' grid.Cell(0, 8).Text = "FileName"
...
On Error GoTo ERR_GET:
    ...
    If FileExists(USER_SCHEDULED_STD_PATH & FileName) Then
...
CloseSettingDataFile
    ...
    'SaveSettingData SettingName, "ScheduledSTD", "RecipeIndex", Index
        ...
        If USER_PATH <> USER_SCHEDULED_STD_PATH Then USER_PATH = USER_SCHEDULED_STD_PATH
     
                IndexRecipe = GetSettingData(FileName, "ScheduledSTD", "RecipeIndex", 0)
                
               
                uPrep.Note = GetSettingData(FileName, "iRecipeForSTDPreparation", "Note", "")
            ...
            uPrep.QtyProduced = GetSettingData(FileName, "ScheduledSTD", "QtyProduced", 0)
        End If
