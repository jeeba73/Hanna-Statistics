...
Option Explicit
Public Function SetGridSTDPreparation(ByRef Grd1 As Grid) As Boolean
    ...
    With Grd1
      .Rows = 1
        ...
        .Cell(0, 0).Text = "n."
        .Column(0).Width = 0
        .Cell(0, 1).Text = "STDPreparationWay"
        .Column(1).Width = 150
        .Cell(0, 2).Text = "ID"
        .Column(2).Width = 0
...
Public Function SetGridEditSTDPreparation(ByRef Grd As Grid) As Boolean
    ...
    With Grd
        ...
        .Cell(1, 1).Text = "  " & "STDPreparationWay"
        .Cell(2, 1).Text = "  " & "Speed"
        .Cell(3, 1).Text = "  " & "Line"
    ...
    End With
End Function
Public Sub CopySTDPreparationGrd1(ByRef Grd As Grid, Optional ByVal Code As String, Optional bMainForm As Boolean, Optional ByVal sString As String)
Dim i As Integer
Dim t As Integer
     ...
     With dbTabSTDPreparationWay
        ...
        If sString <> "" And Code <> "" Then
            '.Filter = "STDPreparationWay ='" & Replace(Trim(Code), "'", "''") & "'"
             .filter = "STDPreparationWay like '*" & Replace(Trim(Code), "'", "''") & "*'"
        End If
    ...
    With Grd
       .AutoRedraw = False
       For i = 1 To dbTabSTDPreparationWay.RecordCount
       .AddItem "", False
        .Cell(.Rows - 1, 1).Text = IIf(IsNull(Trim(dbTabSTDPreparationWay.fields(1))), "", Trim(dbTabSTDPreparationWay.fields(1)))
        .Cell(.Rows - 1, 2).Text = dbTabSTDPreparationWay!ID
        dbTabSTDPreparationWay.MoveNext
       Next
       .Refresh
       .ReadOnly = True
        .AutoRedraw = True
...
Public Sub CopySTDPreparationGrd2(ByVal Grd2 As Grid, ByVal lId As Long)
Dim i As Integer
Dim t As Integer
    ...
    With Grd2
       ...
       For i = 1 To .Rows - 1
        .Cell(i, 2).Text = IIf(IsNull(Trim(dbTabSTDPreparationWay.fields(i))), "", Trim(dbTabSTDPreparationWay.fields(i)))
       Next
...
Public Sub Grd2_STDPreparation_LeaveCell(ByVal Grd2 As Grid, ByVal Row As Long, ByVal Col As Long, NewRow As Long, NewCol As Long, Cancel As Boolean, ByVal lRow As Long)
...
Public Function SaveDatabaseSTDPreparation(ByVal Grd2 As Grid) As Boolean
Dim rc As Boolean
...
rc = True
    ...
    With dbTabSTDPreparationWay
        .filter = ""
        .filter = "STDPreparationWay='" & MyNewCode & "'"
        If .EOF Then
        ...
        !STDPreparationWay = Trim(Grd2.Cell(1, 2).Text)
...
Public Sub AddComboSTDPreparation(ByVal Combo1 As ComboBox)
    Combo1.Clear
    Combo1.AddItem "STDPreparationWay"
    Combo1.ListIndex = 0
End Sub
