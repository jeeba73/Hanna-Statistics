...
Dim calcRight&, maxRight&, calcBottom&, maxBottom&
    ...
    '6) Add Scrollbars
    'Vertical
    Set UCScrollV = TargetForm.Controls.Add("ChemicalMR.ucScrollBar", UCScrollVName$)
    SetParent UCScrollV.hwnd, Target.hwnd
    UCScrollV.AddedFromCodeINIT_AND_DEFAULTS
    UCScrollV.Style = sFlat
    If (Err.NUMBER <> 0) Then Debug.Print ("ChemicalMR UCScrollV Error: " & Err.Description) 'Capture error on create
    
    'Horizontal
    Set UCScrollH = TargetForm.Controls.Add("ChemicalMR.ucScrollBar", UCScrollHName$)
    ...
    If (Err.NUMBER <> 0) Then Debug.Print ("ChemicalMR UCScrollH Error: " & Err.Description) 'Capture error on prop.set
    Err.Clear
