...
Public Function CopyChemicalProductionData(Optional ByVal FileName As String) As Boolean
    Dim i As Long
    Dim a As Integer
    Dim rc As Boolean
...
Riga = Riga + 4
    ...
    For i = 1 To MeterNumber
        ...
        Call AddValue(Riga + 1, i * 2, GetSettingData(SettingName, "Information QC", "Text1" & 31 + i * 2, ""))
                ...
                For b = 1 To MyCols - 1
                    ...
                    ElseIf t = 11 Or t = 12 Or t = 13 Or t = 14 Then
                        ...
                        Else
                            ...
                            Standard = GetSettingData(SettingName, "Reading QC", "Grd2 Row" & i & " Col" & 1, 0)
...
Dim MaxTest
    ...
    MyChemicalProduction = MyChemicalProductionClean
    ...
    For i = 1 To STDCount
        ...
        MyChemicalProduction.STDtest(t).MaxReadings = GetSettingData(SettingName, "Graph QC", "Standard " & t & " Total Readings", 0)
        MyChemicalProduction.STDtest(t).SelReadings = GetSettingData(SettingName, "Graph QC", "Standard " & t & " Selected Readings", 0)
        MyChemicalProduction.STDtest(t).NumTest = GetSettingData(SettingName, "Graph QC", "Standard " & t & " Total Tests", 0)
        MyChemicalProduction.STDtest(t).SelTest = GetSettingData(SettingName, "Graph QC", "Standard " & t & " Selected Tests", 0)
        MyChemicalProduction.STDtest(t).TotalMean = GetSettingData(SettingName, "Graph QC", "Standard " & t & " Total Average", 0)
        MyChemicalProduction.STDtest(t).SelecMean = GetSettingData(SettingName, "Graph QC", "Standard " & t & " Selected Average", 0)
         
        Call AddValue(Riga + i * 4 + 1, 2, "Total Tests", True)
        Call AddValue(Riga + i * 4 + 2, 2, CStr(MyChemicalProduction.STDtest(t).NumTest))

        Call AddValue(Riga + i * 4 + 1, 3, "Total Readings", True)
        Call AddValue(Riga + i * 4 + 2, 3, CStr(MyChemicalProduction.STDtest(t).MaxReadings))
        
        Call AddValue(Riga + i * 4 + 1, 4, "Total Average", True)
        Call AddValue(Riga + i * 4 + 2, 4, MyChemicalProduction.STDtest(t).TotalMean & " " & MeasurementUnit)
        
        Call AddValue(Riga + i * 4 + 1, 5, "Selected Tests", True)
        Call AddValue(Riga + i * 4 + 2, 5, CStr(MyChemicalProduction.STDtest(t).SelTest))
        
        Call AddValue(Riga + i * 4 + 1, 6, "Selected Readings", True)
        Call AddValue(Riga + i * 4 + 2, 6, CStr(MyChemicalProduction.STDtest(t).SelReadings))
        
        Call AddValue(Riga + i * 4 + 1, 7, "Selected Average", True)
        Call AddValue(Riga + i * 4 + 2, 7, MyChemicalProduction.STDtest(t).SelecMean & " " & MeasurementUnit)
    ...
    For i = 0 To nRows - 1
        For t = 1 To nCols - 1
            ...
            ElseIf t = 7 Then
                If i = 0 Then
                    Call AddValue(Riga + i, 1 + t - 3, GetSettingData(SettingName, "Evaluation QC", "Results Grid Standard (" & i & ")  Column " & t, ""), True)
                Else
...
add:
                            ...
                            OtherParameterFormula(OtherCount) = Trim(!ParameterFormula)
