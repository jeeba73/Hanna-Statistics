...
Dim rc As Boolean
    ...
    rc = True
     ...
     With RecipeQC
        ...
        USER_PATH = USER_SCHEDULED_STD_PATH
        ...
        PassDate = GetSettingData(.SettingName, "ScheduledSTD", "PassToQC Date", "")
        CloseSettingDataFile
    
        If PassDate = "" Then
            SaveSettingData .SettingName, "ScheduledSTD", "PassToQC", True
            SaveSettingData .SettingName, "ScheduledSTD", "PassToQC Operator", .Operator
            SaveSettingData .SettingName, "ScheduledSTD", "PassToQC Date", .Date
        End If
    End With
    ...
    With Grid
        ...
        With dbTabPreparation
            ...
            For i = 1 To MaxCount
                ...
                Grid.Cell(Grid.Rows - 1, 3).Text = IIf(IsNull(Trim(!Description)), "", Trim(!Description))
...
Dim rc As Boolean
    ...
    ElseIf FileExists(USER_SCHEDULED_STD_PATH & "Data\" & SettingName) Then
        USER_PATH = USER_SCHEDULED_STD_PATH & "Data\"
    Else
        rc = False
        GoTo ERR_END
    End If
...
Dim RecipeQC As QCType
    ...
    If FileExists(USER_SCHEDULED_STD_PATH & SettingName) Then
    ...
    SaveSettingData SettingName, "iPreparation", "bOpen", False, USER_SCHEDULED_STD_PATH
    SaveSettingData SettingName, "QC Closed", "Manually", True, USER_SCHEDULED_STD_PATH
    SaveSettingData SettingName, "QC Closed", "Date", Now(), USER_SCHEDULED_STD_PATH
    SaveSettingData SettingName, "QC Closed", "Operator", MyOperatore.Name, USER_SCHEDULED_STD_PATH
    
    CloseSettingDataFile
    
    FileCopy USER_SCHEDULED_STD_PATH & SettingName, USER_SCHEDULED_STD_PATH & "data\" & SettingName
    Kill USER_SCHEDULED_STD_PATH & SettingName
...
Public Function MoveRecipeInSTDPreparationDatabase(ByVal PreparationFileName As String, ByVal PreparationID As String, ByVal bIsMix As Boolean) As Boolean
...
cont:
    ...
    If Not (bIsMix) Then
        ...
        If SetDatabaseTabSTDPreparation(RecipeQC) Then
    ...
    Else
        ...
        If rfpSettingName <> "" Then
            ...
            ElseIf FileExists(USER_STD_PREPARATION_PATH & rfpSettingName) Then
                RFPFILE_PATH = USER_STD_PREPARATION_PATH
            End If
            
            CloseSettingDataFile
            
            With userRfp
                SaveSettingData rfpSettingName, "iRecipeForSTDPreparation", "PreparationDate", .PrepDate, RFPFILE_PATH
                SaveSettingData rfpSettingName, "iRecipeForSTDPreparation", "PrepWeek", .PrepWeek, RFPFILE_PATH
                SaveSettingData rfpSettingName, "iRecipeForSTDPreparation", "numPrepWeek", .numPrepWeek, RFPFILE_PATH
                SaveSettingData rfpSettingName, "iRecipeForSTDPreparation", "ExpDate", .ExpDate, RFPFILE_PATH
            End With
...
ERR_END:
    On Error GoTo 0
    CloseSettingDataFile
    MoveRecipeInSTDPreparationDatabase = rc
    Exit Function
ERR_SET:
    rc = False
    Resume Next
