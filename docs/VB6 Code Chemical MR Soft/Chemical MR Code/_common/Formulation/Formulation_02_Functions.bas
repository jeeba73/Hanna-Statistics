...
Public Function AddCodeInRecipeForSTDPreparationGrid(ByVal Grid1 As Grid, ByVal HannaCode As String, ByVal bSalta As Boolean) As Boolean
Dim rc As Boolean
Dim i As Integer
...
cont:
            ...
            For i = LBound(uHannaCode) To UBound(uHannaCode)
                ...
                .Cell(.Rows - 1, 6).Text = "" ' uHannaCode(i).Code
...
ERR_END:
    ...
    AddCodeInRecipeForSTDPreparationGrid = rc
    Exit Function
ERR_ADD:
    rc = False
...
Public Function SetHannaGridSpecific(ByVal Grd As Grid, Optional ByVal bSTDPreparation As Boolean)
...
Public Function AddRecipeInRecipeForSTDPreparationGrid(ByVal Grid1 As Grid, ByVal Grid2 As Grid, ByVal Grid4 As Grid, ByRef iRecipe() As RecipeType) As Boolean
Dim rc As Boolean
Dim i As Integer
Dim t As Integer
...
Dim uRFSTDPreparation As RecipeType
Dim uRFSTDPreparationClean As RecipeType
On Error GoTo ERR_ADD:
    ...
    With Grid1
        ...
        For i = 1 To .Rows - 1
            ...
            If Len(strRecipe) > 0 Then
                Call SplitTextString("", strRecipe, Recipes(), Quanti)
                If Quanti > 0 Then
...
ERR_END:
    On Error GoTo 0
    AddRecipeInRecipeForSTDPreparationGrid = rc
    Exit Function
ERR_ADD:
    rc = False
    MsgBox err.Description
...
If AllCount > 0 Then
       ...
       With Grid2
            ...
            For i = 0 To AllCount - 1
                ...
                uRFSTDPreparation = uRFSTDPreparationClean
                ...
                If MaxRecipes - 1 >= i Then
                    ...
                    If iRecipe(i + 1).Code = AllRecipes(i) Then
                        ...
                        uRFSTDPreparation = iRecipe(i + 1)

                    Else
                    

                        Call SetMyRecipeByCode(AllRecipes(i), uRFSTDPreparation)
                         ReDim Preserve iRecipe(MaxRecipes + 1 + i)
                         iRecipe(MaxRecipes + 1 + i) = uRFSTDPreparation
                ...
                If uRFSTDPreparation.Code = "" Then GoTo cont2
 
                .AddItem "", False
                
                .Cell(.Rows - 1, 1).Text = uRFSTDPreparation.Code
                .Cell(.Rows - 1, 2).Text = uRFSTDPreparation.Description
                .Cell(.Rows - 1, 3).Text = uRFSTDPreparation.Line
                .Cell(.Rows - 1, 4).Text = uRFSTDPreparation.MultipleToProduce
                .Cell(.Rows - 1, 5).Text = uRFSTDPreparation.UmMultiple
                .Cell(.Rows - 1, 6).Text = uRFSTDPreparation.TotalRecipe
                .Cell(.Rows - 1, 7).Text = uRFSTDPreparation.Mix
                .Cell(.Rows - 1, 8).Text = uRFSTDPreparation.Density
                .Cell(.Rows - 1, 9).Text = uRFSTDPreparation.MinQty & " " & uRFSTDPreparation.UmMax
                .Cell(.Rows - 1, 10).Text = uRFSTDPreparation.MaxQty & " " & uRFSTDPreparation.UmMax
                .Cell(.Rows - 1, 11).Text = uRFSTDPreparation.MinQty2 & " " & uRFSTDPreparation.UmMinQty
                .Cell(.Rows - 1, 12).Text = uRFSTDPreparation.Multiple
                .Cell(.Rows - 1, 13).Text = uRFSTDPreparation.UmMultiple
                .Cell(.Rows - 1, 14).Text = uRFSTDPreparation.Exp
                .Cell(.Rows - 1, 15).Text = uRFSTDPreparation.Procedure
                .Cell(.Rows - 1, 16).Text = uRFSTDPreparation.Rev
                .Cell(.Rows - 1, 17).Text = uRFSTDPreparation.NoteRev
                .Cell(.Rows - 1, 18).Text = uRFSTDPreparation.bIsMix
...
On Error GoTo ERR_ADD:
    ...
    If F_MsgBox.DoShow("Warning : Delete all quantity in Hanna Codes Table?", "RecipeForSTDPreparation") Then
