...
Begin VB.Form FormProductionDatabaseHistory 
   ...
   Begin FlexCell.Grid GridDatabase 
      ...
      MultiSelect     =   0   'False
   End
   Begin ChemicalMR.ctlCalendar ctlCalendar1 
      Height          =   6960
      Left            =   11760
      TabIndex        =   15
      Top             =   1680
      ...
      _ExtentX        =   14314
      _ExtentY        =   12277
      ShowLastMonthButton=   0   'False
      ShowNextMonthButton=   0   'False
      ShowLastMonthDays=   0   'False
      ShowNextMonthDays=   0   'False
      ShowTodayLabel  =   0   'False
      ColorBackgroundHeader=   6571523
      ColorForegroundHeader=   16777215
      ColorSelectedBack=   6571523
      ColorSelectedFore=   0
      ColorToday      =   255
...
Private bClosedSTDPreparation As Boolean
Private MyIndexRecord As Integer
Private lRow As Long
Private IndexOpenClosedLot As Integer


Private Sub cmbLineSTDPreparation_Click()
GlobalSearch
End Sub
...
Call SetDatabaseSTDPreparationTable(GridDatabase)
Call SetDatabaseSTDPreparationHistory(Grid2)
Call SetLine(cmbLineSTDPreparation, True)

    

    STDPreparationID = 0
    STDPreparationFileName = ""
    MyIndexRecord = 3
    bMeanValue = False
    bClosedSTDPreparation = False
...
If lRow = 0 Then Exit Sub
    If STDPreparationFileName <> "" Then
    
        If FileExists(USER_STD_PREPARATION_PATH & STDPreparationFileName) Then
            USER_PATH = USER_STD_PREPARATION_PATH
        ElseIf FileExists(USER_STD_PREPARATION_PATH & "data\" & STDPreparationFileName) Then
        ...
        frmSTDPreparation.Left = Me.Left
        frmSTDPreparation.Top = Me.Top
        frmSTDPreparation.WindowState = Me.WindowState
        Call frmSTDPreparation.DoShow(STDPreparationFileName, STDPreparationID)
    End If
...
Dim MyexcelName As String
    ...
    Select Case Index
        Case 0
            cmbLineSTDPreparation.ListIndex = 0
            Combo1.ListIndex = 0
            Combo1_Click
        ...
        Case 1
            ...
            Dim ProdWeekFileArray() As String
            Dim i As Integer
            Dim ProdCount As Integer
            
            ProdWeek = FormatDataWeek(Text1(0))
            ' export LOT Excel
            If GridDatabase.Rows > 1 And ProdWeek <> "" Then
                ...
                For i = 1 To GridDatabase.Rows - 1
                    ...
                    ProdWeekFileArray(i) = Trim(GridDatabase.Cell(i, 13).Text)
                    
                Next
         
                ExcelFilename = "PROD_WK_" & FormatNomeFile(Trim(ProdWeek) & ".LINE_" & Trim(cmbLineSTDPreparation)) & ".xls"
                ...
                Call EsportaSTDPreparationWeekExcel(ProdWeekFileArray, ExcelFilename, ProdCount, ProdWeek, cmbLineSTDPreparation)
            End If
...
err_canc:
        ...
        Case 4
            ...
            ExcelName = FormatNomeFile("STDPreparation List." & FormatDataLAT(Now()))
            
            If F_InputBox.DoShow("Please Set Excel Name", "STDPreparation To Excel", , , , ExcelName) Then
            ...
            End If
        Case 5
            ' View STDPreparation History
            Call ViewSTDPreparationHistory

    End Select
End Sub

Private Sub ViewSTDPreparationHistory()
    ...
    If Grid2.Visible = True Then
        ...
        Label2(5) = "STDPreparation Specifics"
    Else
    
        frSpecifics.Visible = True
        Me.BackColor = &HA0A0A0
        Call FillSTDPreparationSpecifics
...
Dim rc As Boolean
rc = IIf(GridDatabase.Column(4).Width = 0, True, False)
...
Dim t As Integer
    ...
    MyDa = lbDataFiltro(0)
    MyA = lbDataFiltro(1)
    Call FillTabellaTutte(Grd, MyPeriodo, Combo1, Text1(0), MyDa, MyA, IndexOpenClosedLot, cmbLineSTDPreparation)
    IndexTabella = 1
...
If FirstRow > 0 Then
    ...
    STDPreparationID = GridDatabase.Cell(FirstRow, 14).Text
    STDPreparationFileName = GridDatabase.Cell(FirstRow, 13).Text
    bClosedSTDPreparation = GridDatabase.Cell(FirstRow, 15).Text
    PicMenu(4).Visible = True
    
    USER_PATH = IIf(bClosedSTDPreparation, USER_STD_PREPARATION_PATH & "Data\", USER_STD_PREPARATION_PATH)
   


    PicMenu(5).Visible = IsSTDPreparationStarted(STDPreparationID)
Else
End If
End Sub
            ...
            Do
                With Grd
                   ...
                   ' .Cell(0, 13).Text = ""
                    ...
                    .Cell(.Rows - 1, 1).Text = IIf(IsNull(Trim(dbTabSTDPreparation!Line)), "", Trim(dbTabSTDPreparation!Line))
                    .Cell(.Rows - 1, 2).Text = IIf(IsNull(Trim(dbTabSTDPreparation!startDate)), "", Trim(dbTabSTDPreparation!startDate))
                    .Cell(.Rows - 1, 3).Text = IIf(IsNull(Trim(dbTabSTDPreparation!PrepWeek)), PreparationWeek(dbTabSTDPreparation!startDate), Trim(dbTabSTDPreparation!PrepWeek))
                    .Cell(.Rows - 1, 4).Text = IIf(IsNull(Trim(dbTabSTDPreparation!HannaCode)), "", Trim(dbTabSTDPreparation!HannaCode))
                    .Cell(.Rows - 1, 5).Text = IIf(IsNull(Trim(dbTabSTDPreparation!DataRecipe)), "", Trim(dbTabSTDPreparation!DataRecipe))
                    .Cell(.Rows - 1, 6).Text = IIf(IsNull(Trim(dbTabSTDPreparation!Recipe)), "", Trim(dbTabSTDPreparation!Recipe))
                    .Cell(.Rows - 1, 8).Text = IIf(IsNull(Trim(dbTabSTDPreparation!PlanningReference)), "", Trim(dbTabSTDPreparation!PlanningReference))
                    .Cell(.Rows - 1, 9).Text = IIf(IsNull(Trim(dbTabSTDPreparation!PrepDate)), "", Trim(dbTabSTDPreparation!PrepDate))
                    .Cell(.Rows - 1, 10).Text = IIf(IsNull(Trim(dbTabSTDPreparation!PrepWeek)), "", (Trim(dbTabSTDPreparation!PrepWeek)))
                    .Cell(.Rows - 1, 11).Text = IIf(IsNull(Trim(dbTabSTDPreparation!numPrepWeek)), "", Trim(dbTabSTDPreparation!numPrepWeek))
                    .Cell(.Rows - 1, 12).Text = IIf(IsNull(Trim(dbTabSTDPreparation!Note)), "", Trim(dbTabSTDPreparation!Note))
                    .Cell(.Rows - 1, 13).Text = IIf(IsNull(Trim(dbTabSTDPreparation!FileName)), "", Trim(dbTabSTDPreparation!FileName))
                    .Cell(.Rows - 1, 14).Text = dbTabSTDPreparation!ID
                    .Cell(.Rows - 1, 15).Text = dbTabSTDPreparation!bClosed
                    If dbTabSTDPreparation!bClosed Then
                        .Cell(.Rows - 1, 16).Text = IIf(IsNull(Trim(dbTabSTDPreparation!CloseDate)), .Cell(.Rows - 1, 2).Text, Trim(dbTabSTDPreparation!CloseDate))
                    End If
                    If dbTabSTDPreparation!bClosed Then
                    ...
                    If dbTabSTDPreparation!ExcelDone Then
...
Private Function CancellaTab() As Boolean
    ...
    If STDPreparationID > 0 Then
        If F_MsgBox.DoShow(("Delete Selected Record ?"), "Database", , ("Delete"), ("Exit")) Then
            ...
            If CancellaRecord(STDPreparationID) Then
                Text1(0) = ""
                GlobalSearch
...
Private Function CancellaRecord(ByVal STDPreparationID As Long) As Boolean
Dim rc As Boolean
    ...
    With dbTabSTDPreparation
        ...
        Else
            ...
            If STDPreparationFileName <> "" Then
                If FileExists(USER_PATH & STDPreparationFileName) Then Kill USER_PATH & STDPreparationFileName
            End If
        End If
...
Private Sub FillSTDPreparationSpecifics()
Dim i As Integer
...
With Grid2
    ...
    With dbTabProdHistory
        .filter = ""
        .filter = "STDPreparationID='" & STDPreparationID & "'"
        .MoveFirst
