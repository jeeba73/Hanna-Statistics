         ...
         Begin VB.Frame frInside 
            ...
            Begin VB.Frame frCommandInside 
               ...
               Begin VB.Label lbCommandInside 
                  Alignment       =   2  'Center
                  BackStyle       =   0  'Transparent
                  Caption         =   "Print QRCode Close Prod."
                  BeginProperty Font 
                     Name            =   "Century Gothic"
                     Size            =   9
                     Charset         =   0
...
Private uProductionClean As RecipeForProduction
...
Private bProductionClosed  As Boolean
...
Public Function DoShow(Optional ByVal FileName As String, Optional ByVal Production_ID As Long) As Boolean
    ...
    bIfDataPath = IIf(USER_PATH = USER_PRODUCTION_PATH, True, False)

    SettingName = FileName
        
    If SettingName = "" Then
        '----------------------------------------------
        ' è una production senza nessuna preparation!
        '----------------------------------------------
        Call SetProductionNoPreparation
    End If
...
Dim sString As String
 ...
 Recipe = Grid1.Cell(lRowHanna, 10).Text
   sString = "Enter or Modify Qty to Produce"
    
            If F_InputBox.DoShow(sString, HannaCode & " | " & Recipe, , , , Qty, , True, Me.Top) Then
            
                
                If IsNumeric(Qty) Or Qty = "" Then
                
                    If bImportata Then bImportata = False
                    uHannaCodes(lRowHanna).QtyToProduce = Qty
                    Grid1.Cell(lRowHanna, 6).Text = Qty
                    Grid1.Cell(lRowHanna, 6).Alignment = cellCenterCenter
                    Grid1.Refresh
                    Grid1.AutoRedraw = True
                    'Call UpdateValue(lRowHanna)
                End If
...
If bIfDataPath Then AddProduction
End Sub
...
Private Sub Grid1_SelChange(ByVal FirstRow As Long, ByVal FirstCol As Long, ByVal LastRow As Long, ByVal LastCol As Long)
    ...
    frCommandInside(16).Visible = False
    ...
    If FirstRow > 0 Then
        ...
        lRowHanna = FirstRow
         ...
         frCommandInside(6).Visible = IIf(HannaCode <> "", True, False)
         frCommandInside(5).Visible = IIf(HannaCode <> "", True, False)
         ...
         If Grid1.Cell(lhannaCodeRow, 7).Text > 0 Then
             frCommandInside(16).Visible = True
         Else
             frCommandInside(16).Visible = False
         End If
...
If FirstRow > 0 Then
    ...
    lAcquisitionIndex = FirstRow 'Grid2.Cell(FirstRow, 11).Text
    frCommandInside(11).Visible = True ' Not (bProductionClosed)
...
Private Sub ImViewRecipes_Click(Index As Integer)
    Select Case Index
        Case 0
            ...
            Call ViewHannaCodeInProduction(uHannaCodes, Grid1, True)
        Case 1
            Call ViewHannaCodeInProduction(uHannaCodes, Grid1, False)
...
Private Sub InitForm()
   ...
   ' Set Grid(6) = Grid7
    ...
    Call SetAllProductionGrid(Grid())
    Call SetColumnWidth
...
With uProduction
        ...
        If .PrepWeek = "" Then .PrepWeek = PreparationWeek(Now())
            ...
            PopupMessage 2, "Please enter Number Preparation Week", , , "Production"
            If F_InputBox.DoShow("Number Preparation Week", "Production", , , , strPrep) Then
                .numPrepWeek = strPrep
            End If
...
Select Case Index
    Case 0
        If F_MsgBox.DoShow("Quit Production?") Then
            
            If Grid2.Rows > 1 Then
                If bIfDataPath Then
                    If F_MsgBox.DoShow("Save Production?") Then
...
Dim UserHannaCode As String
    ...
    Case 7
        ' exit production
        Call SetProduction(False)
    Case 8
        ' save production
        Call SetProduction(True)
    Case 9
    ...
    Case 13
        If SettingName <> "" Then
            ...
            Call GetProductionNotes(Grid4, SettingName)
        ...
        Else
            PopupMessage 2, "Please save Production first..."
        End If
            
    Case 14
        PBContainer.Top = 0
    Case 15
            If F_MsgBox.DoShow("Print Production QRCode?", HannaCode) Then
                Call PrintLabelProduction(SettingName, ProductionID, HannaCode)
            End If
     Case 16
        
            UpdateHannaCodeFromGrid
        
            If F_MsgBox.DoShow("Print Close Production QRCode?", HannaCode) Then
                Call PrintLabelCloseProduction(SettingName, ProductionID, HannaCode, lRowHanna)
            End If
...
Dim rc As Boolean
    ...
    Call AddHannaCodeInProduction(HannaCode)
...
Private Sub AddHannaCodeInProduction(ByVal HannaCode As String)
Dim i As Integer
Dim rc As Boolean
...
On Error GoTo ERR_ADD:
   ...
   ' uRecipe = uRecipe
    ...
    Call AddCodeInProductionGrid(Grid1, HannaCode, uHannaCodes)
    ...
    ' carico la Gird1
    '-------------------------------------------
    Call FillGridProductionFromFile(Grid1, uProduction, 1)
    ...
    If SettingName = "" Then
        ...
        PopupMessage 2, "(1) Check and Fill Recipe for Production Details" & vbCrLf & "(2) SAVE production!! " & vbCrLf & "(3) Add Production acquisition."
    End If
...
Private Function SetProduction(ByVal rc As Boolean)
Dim mrc As Boolean
...
Private Function AddProduction()

    If ProductionID = 0 Then
        MessageInfoTime = 2500
        PopupMessage 2, "Please Save Info Production data...", , True, "Production"
        Exit Function
    End If
    ClearProduction
    frInside(2).ZOrder
...
Dim rc As Boolean
 rc = GetProductionFromFile
 
 bImportata = rc
 
End Sub

Private Function GetProductionFromFile() As Boolean
Dim i As Integer
Dim rc As Boolean
...
rc = True
    ...
    uProduction = uProductionClean
        ...
        Else
            rc = False
            PopupMessage 2, "No File production found!!", , True, SettingName
            Exit Function
    ...
    If FileExists(USER_PATH & SettingName) = False Or SettingName = "" Then
        ...
        PopupMessage 2, "Warning : File non found! ", , True, "Production"
        rc = False
        GoTo ERR_END
    End If
    ...
    If bIfDataPath Then
        ...
        Else
            ...
            bProductionClosed = True
            blTable.Visible = False
            blTable = "Production : Closed"
            lbLine = "Date : " & GetProductionDate(ProductionID)
            
            frExcel.Move frCommandInside(0).Left, frCommandInside(0).Top, frCommandInside(0).Width, frCommandInside(0).Height
            frExcel.Visible = True
    
    End If
    
    
    Call ProductionGetSetting(uProduction, SettingName)
    ...
    With uProduction
       ...
       ' txFormulation(7) = .ExpDate
        ...
        ' Preparation Lot.!!!
        txFormulation(7) = .PreparationLot
    ...
    Call FillGridProductionFromFile(Grid1, uProduction, 1)
    Call FillGridProductionFromFile(Grid2, uProduction, 2)
   ' Call FillGridProductionFromFile(Grid3, uProduction, 3)
...
ERR_END:
    ...
    On Error GoTo 0
     uRecipe = uProduction.Recipes
    lbWait.Visible = False
    lbLine.Visible = True
    GetProductionFromFile = rc
    Exit Function
ERR_GET:
    rc = False
    Resume Next
...
End If
    ...
    Selected = lbAcquisition(Index) ' "Production"
    Answer = txAcquisition(Index)
    sString = "Please Enter Production Detail"
    ComboMachine.Visible = False
...
Dim rc As Boolean
    ...
    With Grid2
        ...
        Call ProductionAddNewRowInAcquisition(Grid2, userAcquisition)
    ...
    uHannaCodes(lhannaCodeRow).QtyProduced = uProduction.HannaCodes(lhannaCodeRow).QtyProduced
...
Dim AcquisitionsCount As Integer
    ...
    AcquisitionsCount = GetAcquisitionCount(ProductionID)
    ...
    uHannaCodes(lhannaCodeRow).DateProd = userAcquisition.DateProd
    
    If uHannaCodes(lhannaCodeRow).QtyProduced = "" Then uHannaCodes(lhannaCodeRow).QtyProduced = "0"
    
    uHannaCodes(lhannaCodeRow).QtyProduced = CheckQtyProducedAcquisition(ProductionID, txAcquisition(0)) + txAcquisition(10)
    userAcquisition.Index = AcquisitionsCount
    
    
    uProduction.HannaCodes(lhannaCodeRow) = uHannaCodes(lhannaCodeRow)
    
    uProduction.WeekProd = userAcquisition.WeekProd
...
Index = 0
    ...
    If Path = "" Then Path = USER_PRODUCTION_PATH
...
If AcquisitionID > 0 And AcquisitionHannaCode <> "" Then
    ...
    If F_MsgBox.DoShow("Delete Acquisition Code : " & AcquisitionHannaCode & vbCrLf & "Qty Produced : " & Trim(PadString(AcquisitionQty)), RecipeCode, True) Then
        IndexCode = GetIndexHannaCodeFromFile(SettingName, AcquisitionHannaCode, USER_PATH)
    Else
        Exit Function
    End If
    '-----------------------------------
...
cont:
    ...
    Call DeleteRowInTabProductionAcquisition(AcquisitionID)
    ...
    uProduction.HannaCodes(IndexCode) = uHannaCodes(IndexCode)
    
    Call FillGridProductionFromFile(Grid1, uProduction, 1)
    
    uHannaCodes(IndexCode).QtyProduced = uProduction.HannaCodes(IndexCode).QtyProduced
...
Private Function SaveProduction()
...
If Grid1.Rows > 1 Then
    ...
    End With
    '-------------------------------------------
    ' aggiorna QtyProduced
    '-------------------------------------------
    Call UpdateHannaCodeFromGrid
    '-------------------------------------------
    ' Salva e aggiorna TabProduction
    '-------------------------------------------
    Call AggiornaTabProduction(ProductionID, uProduction)
    '-------------------------------------------
    ' Salva e aggiorna File
    '-------------------------------------------
    Call ProductionSaveSetting(uProduction, SettingName)
    
    
    PopupMessage 2, "Production correctly Saved", , , "Production"
    
Else
    
    PopupMessage 2, "Please Select HannaCode first and fill Recipe for Production Details....", , True, "Production"
    
End If
End Function


Private Sub SetProductionNoPreparation()
...
SettingName = FormatNomeFile(Trim(uHannaCodes(0).Code) & "." & Trim(uHannaCodes(0).Line) & "." & txFormulation(1) & "." & txFormulation(2) & "." & txFormulation(3)) & "." & USER_ESTENSIONE_RFP
...
Else
    ...
    ExcelFilename = "PROD_" & FormatNomeFile(Trim(uHannaCodes(1).Recipe) & "." & Trim(uHannaCodes(1).Line) & "." & txFormulation(2) & "." & txFormulation(6) & ".LOT_" & uHannaCodes(1).LotNumber) & ".xls"
    ...
    Call EsportaProductionExcel(SettingName, ExcelFilename, uProduction)
End If
...
With dbTabProductionNotes
    ...
    Else
        If F_MsgBox.DoShow("Delete Note?", "Production Notes", , "Delete", "Exit") Then
            .Delete
            .Update
        Else
            rc = False
...
With dbTabProductionNotes
    ...
    Else
        ...
        If F_MsgBox.DoShow("Note Date : " & OldDate & " already exsists.", "Add Production Note", , "Add", "Exit") Then
            .AddNew
        Else
            AddNotes = False
            Exit Function
...
Private Sub frExcel2_Click()
    If SettingName <> "" Then
        If Grid4.ExportToExcel(USER_DESKTOP & "\" & SettingName & "_ProductionNote_History.xls", True, True) Then
            MessageInfoTime = 2500
...
Dim rc As Boolean
          ...
          Lot = txAcquisition(5)
          rc = InputLotNumber(uHannaCodes(lhannaCodeRow).Code, Lot)
        ...
        txAcquisition(5).BackColor = vbWhite
...
rc = True
    ...
    If F_InputBox.DoShow("Please enter Lot number", strHannaCode, , , , Lot, , True, Me.Top) Then
    ...
    If CheckProductionLot(Lot, uHannaCodes(lhannaCodeRow).Line, False, uProduction) Then
           
           uHannaCodes(lhannaCodeRow).LotNumber = Lot
    Else
        rc = False
    End If
...
Private Function UpdateHannaCodeFromGrid()
Dim i As Integer
Dim Code As String
    ...
    If Grid1.Rows < 1 Then Exit Function
        ...
        For i = 1 To Grid1.Rows - 1
            With uProduction.HannaCodes(i)
                ...
                Code = Trim(Grid1.Cell(i, 1).Text)
                If LCase(.Code) = LCase(Code) Then

                    .QtyToProduce = Trim(Grid1.Cell(i, 6).Text)
                    .QtyProduced = Trim(Grid1.Cell(i, 7).Text)
