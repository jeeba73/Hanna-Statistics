...
Public Function GetRecipeExp(ByVal Recipe As String) As Double
    Dim ExpString As String
    
    With dbTabRecipe
        .filter = ""
        .filter = "Code='" & Replace(Recipe, "'", "''") & "'"
        If .EOF Then
            GetRecipeExp = 0
            Exit Function
        Else
            ExpString = CheckDot(IIf(IsNull(Trim(!Exp)), 0, Trim(!Exp)))
        End If
    
        ExpString = IIf(IsNull(Trim(ExpString)), 0, CheckDot(IIf(IsNull(Trim(!Exp)), 0, Trim(!Exp))))
    End With
    
    GetRecipeExp = CDbl(ExpString)
End Function
...
Public Function GetLastImport(ByRef Frm As Form, ByVal Index As Integer)
Dim rc As Boolean
Dim str As String
Dim Path As String
Dim Name As String
Dim strDate As String
    str = GetSetting(App.Title, "ImportExcel", "FileName" & Index, "")
    strDate = GetSetting(App.Title, "ImportExcel", "Date" & Index, "")
    SplitPathFile str, Path, Name
    If str <> "" Then
        Frm.Label11(0) = "File Name : " & Name
        Frm.Label11(1) = "Last Import Date : " & strDate
        rc = True
    Else
        rc = False
        Frm.Label11(0) = ""
        Frm.Label11(1) = ""
    End If
    Frm.Label11(0).Visible = rc
    Frm.Label11(1).Visible = rc
End Function
...
End If
ERR_END:
    If dbChemicalProduction.State = 0 Then GoTo apri
