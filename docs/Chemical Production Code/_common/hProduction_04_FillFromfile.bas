...
Public Function FillGridProductionFromFile(ByVal Grid As Grid, ByRef iProduction As RecipeForProduction, ByVal Index As Integer)
...
Dim i As Integer
    ...
    Select Case Index
        Case 1
            ' Hanna Codes
            Call GetCodeGrid(Grid, iProduction)
        Case 2
            ' Acquisition
            Call GetAcquisitionGrid(Grid, iProduction)
    End Select
...
Private Sub GetCodeGrid(ByVal Grid As Grid, ByRef iProduction As RecipeForProduction)
Dim i As Integer
Dim t As Integer
      ...
      .Rows = 1
        ...
        With iProduction
            ...
            For i = 1 To HannaCount
                 ...
                 .HannaCodes(i).QtyProduced = CheckQtyProducedAcquisition(.ProductionID, .HannaCodes(i).Code)
                ...
                If CDbl(.HannaCodes(i).QtyProduced) > 0 And CDbl(.HannaCodes(i).QtyToProduce) > 0 Then
                    ...
                    Variance = PercStr & FormatNumber(VarDbl, 2) & " %"
                ...
                Grid.Cell(Grid.Rows - 1, 11).Text = .HannaCodes(i).LotNumber
                
                Grid.Cell(Grid.Rows - 1, 12).Text = .HannaCodes(i).Mix1 & IIf(Len(.HannaCodes(i).Mix2) > 0, ";" & .HannaCodes(i).Mix2, "")
        ...
        End With
ERR_END:
         Call SetProductionHannaGridSpecific(Grid)
...
Public Function SetProductionHannaGridSpecific(ByVal Grd As Grid, Optional ByVal bProduction As Boolean)
...
With Grd
    For t = 1 To .Rows - 1
        ...
        .Cell(t, 11).BackColor = vbColorResults
...
Private Sub GetAcquisitionGrid(ByVal Grid As Grid, ByRef uProduction As RecipeForProduction)
Dim i As Integer
Dim t As Integer
...
On Error GoTo ERR_GET:
    ...
    With Grid
      ...
      If uProduction.ProductionID > 0 Then
        ...
        With uProduction.HannaCodes(i)
            ...
            If .AcquisitionCount > 0 Then
                For t = 1 To .AcquisitionCount
                    Call ProductionAddNewRowInAcquisition(Grid, .Acquisitions(t))
                Next
            End If
        End With
    Next
...
ERR_GET:
    ...
    Resume Next
End Sub
Public Function ProductionAddNewRowInAcquisition(ByRef Grid2 As Grid, ByRef iAcquisition As ProdAcquisition)
Dim i As Integer
With Grid2
