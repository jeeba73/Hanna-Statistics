        ...
        With dbTabFrasiH
            ...
            End If
                ...
                For t = 1 To .fields.Count - 2
                    ...
                    sString = Replace(Trim(FoglioExcel.Cells(r, t + 2)), Chr$(13), "")
                    If sString <> "" Then .fields(t) = Left(Trim(sString), 255)
...
Public Function iExcelProductionWay(ByVal mFile As String, ByVal Frm As Form, Optional ByRef DataCounter As Long, Optional ByVal bDeletePreviousRecords As Boolean) As Boolean
Dim rc As Boolean
Dim file_name As String
Dim ClienteID As Long
Dim SedeID As Long
...
ExcelProductionWay = ProductionWayClean
...
On Error GoTo ERR_CREATE_OBJECT
    ...
    Do
        r = r + 1
    Loop Until (FoglioExcel.Cells(r + 1, 4) = "" And FoglioExcel.Cells(r + 2, 4) = "")
    
    ReDim ExcelProductionWay(r)
    
    
    Frm.List1.AddItem Now & " - Loading Production Way  ..."
    ...
    Do
        ...
        ExcelProductionWay(i).Production = Trim(FoglioExcel.Cells(r, 4))
        ExcelProductionWay(i).Line = Trim(FoglioExcel.Cells(r, 6))

        If ExcelProductionWay(i).Production = "" Then
            GoTo END_FN
        End If
        
        With dbTabProductionWay
            .filter = ""
            .filter = "ProductionWay='" & ExcelProductionWay(i).Production & "'" ' and RangeMin='" & ExcelProductionWay(i).RangeMin & "' and RangeMax='" & ExcelProductionWay(i).RangeMax & "'"
            If Not (.EOF) Then
                
                ' ok controllo che sia associata al cliente
                Frm.List1.AddItem Now & " - Production Way  (" & i & ") : " & ExcelProductionWay(i).Production & " ( " & ExcelProductionWay(i).Line & " ) already Exsists... "
            Else
                .AddNew
                NewCode = NewCode + 1
                Frm.List1.AddItem Now & " - Import new Production Way (" & i & ") : " & ExcelProductionWay(i).Production & " ( " & ExcelProductionWay(i).Line & " )"
            End If
                ...
                For t = 1 To .fields.Count - 2
                    ...
                    sString = Trim(FoglioExcel.Cells(r, t + 3))
                    
                    If sString <> "" Then
                        If .fields(t).Name = "Speed" Then
                            .fields(t) = CInt(sString)
                        Else
                            .fields(t) = sString
                        End If
                    End If
                ...
                !DateModified = Now
                Frm.List1.AddItem Now & " - Hanna Production Way (" & i & ") : " & ExcelProductionWay(i).Production & " ( " & ExcelProductionWay(i).Line & " ) Saved... "
                DoEvents
                .Update
        End With
    ...
    DataCounter = i
    Frm.List1.AddItem ""
    Frm.List1.AddItem "n." & NewCode & " New Production Way Records Imported"
    Frm.List1.AddItem "n." & DataCounter & " Excel Code"
    Frm.List1.AddItem ""
...
END_FN:
    ...
    If rc Then
        Dim Path As String
        Call SplitPathFile(file_name, Path)
        SaveSetting App.Title, "ImportExcel", "FileName1", file_name
        SaveSetting App.Title, "ImportExcel", "Date1", Now
        SaveSetting App.Title, "ImportExcel", "Path1", Path
        PopupMessage 2, "Excel Product Classification Import Procedure Finished...."
    End If
    FileExcel.Close False
    Set FileExcel = Nothing

    iExcelProductionWay = rc
