...
On Error GoTo ERR_SET:
    ...
    uRecipe.Description = "Not Found in Database!"
     With dbTabRecipe
        ...
        uRecipe.ProcedureDate = IIf(IsNull(Trim(!ProcedureDate)), "", Trim(!ProcedureDate))
        ...
        uRecipe.Classification = IIf(IsNull(Trim(!Classification)), "", Trim(!Classification))
    End With
...
End Function
Public Function GetRMLocation(ByRef uRMxRecipe As RmxRecipe)
With dbTabRawMaterial
    .filter = ""
    .filter = "Code='" & uRMxRecipe.CHCode & "'"
    If .EOF Then
       
    Else
    
          uRMxRecipe.Cas = IIf(IsNull(Trim(!Cas)), "", Trim(!Cas))
          uRMxRecipe.Specifications.SpecifiedLocation = IIf(IsNull(Trim(!SpecifiedLocation)), "", Trim(!SpecifiedLocation))
          uRMxRecipe.Specifications.Location = IIf(IsNull(Trim(!Location)), "", Trim(!Location))
    ...
    ' bValue = IfAllMixes(RecipeCode)
     With dbTabRMxRecipe
        ...
        For i = RecipeRecordCount To RecipeRecordCount + .RecordCount - 1
          ...
          '  uRMxRecipe(i).Density = IIf(IsNull(Trim(!Density)), 0, CheckDot(Trim(!Density)))
...
Public Function SetPackagingFromGrid(ByRef Packaging() As ProdWay, ByVal Grid As Grid) As Boolean
 Dim PackagingCount As Integer
    ...
    ReDim Packaging(PackagingCount)
        ...
        '.Cell(0, 2).Text = "Line"
        '.Cell(0, 3).Text = "Production Way" // MACHINE
        '.Cell(0, 4).Text =  // MACHINEHead
        '.Cell(0, 5).Text = "Production speed ( pcs/min )"
        '.Cell(0, 6).Text = "Estimated time machine ( h )"
        '.Cell(0, 7).Text = "Estimated Shift machine ( S )"
        
    For i = 1 To PackagingCount
        With Packaging(i)
            .Recipe = Grid.Cell(i, 1).Text
            .Line = Grid.Cell(i, 2).Text
            .Production = Grid.Cell(i, 3).Text
            .Head = IIf(Grid.Cell(i, 4).Text = "", 0, Grid.Cell(i, 4).Text)
            .Speed = IIf(Grid.Cell(i, 5).Text = "", 0, Grid.Cell(i, 5).Text)
            .EstTimeH = IIf(Grid.Cell(i, 6).Text = "", 0, Grid.Cell(i, 6).Text)
            .EsttimeD = IIf(Grid.Cell(i, 7).Text = "", 0, Grid.Cell(i, 7).Text)
