...
Public Function GetRecipeRevision(ByVal Grd As Grid, ByVal Recipe As String, Optional ByRef Description As String) As Boolean
Dim rc As Boolean
Dim i As Integer
Dim t As Integer
    ...
    With Grd
        ...
        With dbTabRecipeRevisionHistory
            .Close
            .Open "SELECT *  FROM TabRecipeRevisionHistory order by RevDate", dbCode, adOpenKeyset, adLockOptimistic, adCmdText
            .filter = ""
            .filter = "Recipe='" & Replace(Recipe, "'", "''") & "'"
            
            '.filter = "Recipe like '*" & Replace(Recipe, "'", "''") & "*'"
            If .EOF Then
            Else
                .MoveLast
